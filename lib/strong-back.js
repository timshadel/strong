...
back.navigate = function (path) {
  try {
    return path.split('.').reduce(function(obj,i){ return obj[i] }, this._translations);
  } catch (e) {
    return undefined;
  }
};
...
back.toDotPath = function(filename) {
  var extractor = new RegExp("^" + this.translations_dir + "\/(.+)_([^_]+)\\.json$");
  return filename.replace(extractor, '$2/$1').replace(/\//g,'.');
}
