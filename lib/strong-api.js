...
api.translate = function(key, options) {
  ...
  var effective_key = [locale, this._context, key].join('.');
...
api.compile = function (template, data) {
  var view_path = data.filename.split('').splice(data.root.length+1).join('').split('.').reverse().splice(1).reverse().join('.')
