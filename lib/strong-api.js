...
api.lookup = function(locale, key) {
  var context = exists(this._context) ? this._context.slice(0) : [];
  ...
  while (context.length > 1 && !exists(template)) {
    template = this.back.navigate(context);
...
api.translate = function(key, options) {
  ...
  var template = this.lookup(locale, key);
...
api.decorator = function(engine) {
  this._engine_delegate = engine;
  return this;
};
...
api.compile = function (template, data) {
  var extractPath = new RegExp("^" + data.root + "\/(.*)\\.[^\\.]*$");
  this._context = data.filename.replace(extractPath, '$1').split('/');
  return this._engine_delegate.compile(template, data);
};
